...
func urlEscapeScreen(win fyne.Window) fyne.CanvasObject {
	transformLabel := widget.NewLabelWithStyle("Escape:",
		fyne.TextAlignLeading, fyne.TextStyle{Bold: true, Italic: true})
	...
	inputEntry := widget.NewMultiLineEntry()
	inputEntry.Wrapping = fyne.TextWrapBreak
	inputEntry.SetPlaceHolder("This entry is for origin data needed to be escape/unescape")

	outputEntry := widget.NewMultiLineEntry()
	outputEntry.Wrapping = fyne.TextWrapBreak
	outputEntry.SetPlaceHolder("This entry is result escape/unescape data")

	transformSlider.OnChanged = func(valFloat float64) {
		if valFloat == transformSlider.Min {
			transformLabel.SetText("Escape")
			inputEntry.SetText("")

		} else if valFloat == transformSlider.Max {
			transformLabel.SetText("Unescape")
			inputEntry.SetText("")
		}
	}
	...
	calcFunc := func(inputStr string) {
		go func(inputStr string) {
			if transformSlider.Value == transformSlider.Min {
				outputEntry.SetText(url.QueryEscape(inputEntry.Text))
			} else if transformSlider.Value == transformSlider.Max {
				urlOrigin, err := url.QueryUnescape(inputEntry.Text)
				if err != nil {
					outputEntry.SetText(err.Error())
					return
				}
				outputEntry.SetText(urlOrigin)
			}
		}(inputStr)
	}
	inputEntry.OnChanged = calcFunc
...
func htmlEscapeScreen(win fyne.Window) fyne.CanvasObject {
	transformLabel := widget.NewLabelWithStyle("Escape:",
		fyne.TextAlignLeading, fyne.TextStyle{Bold: true, Italic: true})
	...
	calcFunc := func(inputStr string) {
		go func(inputStr string) {
			if transformSlider.Value == transformSlider.Min {
				outputEntry.SetText(html.EscapeString(inputEntry.Text))
			} else if transformSlider.Value == transformSlider.Max {
				outputEntry.SetText(html.UnescapeString(inputEntry.Text))
			}
		}(inputStr)
	}
	inputEntry.OnChanged = calcFunc
